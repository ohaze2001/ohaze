...
- 状态: active
- 版本: 2.1.3（`plugins/ohaze/.claude-plugin/plugin.json`）
...
- 核心 skills: `plugins/ohaze/skills/` —— `brainstorming`（fork） / `spec-to-codex-review` / `using-git-worktrees`（fork，含 teardown） / `writing-plans`（fork） / `plan-to-codex-prompt` / `codex-executor` / `finishing`（7 项菜单 + neat 路由内化）
