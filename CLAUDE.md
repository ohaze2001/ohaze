...
- 状态: active
- 版本: 2.0.0（`plugins/ohaze/.claude-plugin/plugin.json`）
...
- 流程序: brainstorm（主仓不落档）→ worktree → 写 spec（worktree 内 commit）→ plan → Codex
...
- 核心 skills: `plugins/ohaze/skills/` —— `brainstorming`（fork） / `using-git-worktrees`（fork，含 teardown） / `writing-plans`（fork） / `plan-to-codex-prompt` / `codex-executor` / `finishing`（6 项菜单 + neat 路由内化）
- 数据契约: `.ohaze/current-ship.json`（ship handoff，权威 schema 在 `commands/ship.md`）+ `.ohaze/review-verdict.json`（审查结论 + ADVERSARIAL/doc_drift）
...
- 下游消费: 被装入任意项目驱动其 feature ship；自身不向外部系统镜像（v2.0.0 起 vault 镜像已剥离）
