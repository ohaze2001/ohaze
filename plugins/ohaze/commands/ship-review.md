...
On successful merge, write ship-result before cleanup:
```bash
cat > <ohaze_dir>/ship-result.json << 'EOF'
...
Finally `rm <main_repo_path>/.ohaze/current-ship.json`.
...
After successful push, write ship-result before cleanup:
```bash
cat > <ohaze_dir>/ship-result.json << 'EOF'
...
Ask: "继续保留 worktree 还是清理?" (1=keep, 2=clean). On clean: `git worktree remove <worktree_path>` + `git branch -D <branch>` from the main checkout.
...
After PR is created, write ship-result before cleanup:
```bash
cat > <ohaze_dir>/ship-result.json << 'EOF'
...
On confirm, write ship-result first (hook reads it before handoff is deleted):
```bash
cat > <ohaze_dir>/ship-result.json << 'EOF'
