...
> 个人 Claude Code 工作流插件：Claude 主线程编排 + Codex 异源执行 + Claude 异源审查 + 自包含收尾。**v2.0.0 起零运行时外部 skill 依赖。**
...
`ohaze` 把多个工具的强项串成一条闭环流水线，**一条 `/ohaze:ship "做什么"` 命令端到端跑完**。v2 流程序：brainstorm（不落档）→ worktree → 写 spec（worktree 内 commit）→ plan → Codex → 异源审查 → 6 项 finishing 菜单。

| 阶段 | 谁干 | 来源 |
|---|---|---|
| 1. brainstorming（文本澄清，终止于设计获批，不落档） | Claude | `ohaze`（fork 自 superpowers v5.1.0，砍 visual companion） |
| 2a. using-git-worktrees（建 worktree + 安全 teardown） | Claude | `ohaze`（fork 自 superpowers v5.1.0 + 新增 teardown cd 主仓） |
| 2b. 在 worktree 内写 spec 并 commit | Claude | `ohaze`（`/ohaze:ship` Phase 2b） |
...
| 7. finishing（6 项菜单 + 收尾链 + 文档收尾） | Claude | `ohaze`（`finishing` skill） |
...
- **流程序 brainstorm → worktree → spec**：brainstorm 主仓内纯对话不落档；建 worktree 后在 worktree 内写 spec + commit。main 全程干净，并行多 ship 不互相污染 ff-merge。
...
- **finishing**：`ohaze:finishing` skill 拥有 Phase 7 —— 项目类型检测（local/remote）+ 6 项菜单 + 收尾链 + doc-finish 内化 neat 路由（CHANGELOG/version/linked_todo + drift 修复 + 待办→ROADMAP + 架构→README/CLAUDE，止于四件套）。
- **第 6 项 conditional**：finishing 菜单第 6 项「修复对抗审查后收尾」**仅当本次有 ADVERSARIAL findings 时出现**，让用户对设计层风险有结构化修复入口。

## 常用命令

| 命令 | 作用 |
|---|---|
| `/ohaze:ship "需求" [--project <abs-path>]` | 端到端：brainstorm → worktree+spec → plan → Codex 后台派发 |
...
# Claude 走 brainstorm（不落档）→ 建 worktree → 在 worktree 写 spec + commit → plan
...
`ohaze:finishing` skill 检测项目类型（`local` / `remote`），给出推荐收尾链，再列菜单（5 或 6 项，第 6 项仅当有 ADVERSARIAL findings 时出现）：
